...
use crate::{
    engine::registry_index::EntityRegistryIndex,
    entity::{
...
impl Engine {
    ...
    pub fn get_point(&self, index: EntityRegistryIndex) -> &EntityPoint {
        match index {
            EntityRegistryIndex::Stable(i) => &self.state.stable_entities.points[i],
            EntityRegistryIndex::Unstable(i) => &self.state.unstable_entities.points[i],
        }
    }

    pub fn get_point_mut(&mut self, index: EntityRegistryIndex) -> &mut EntityPoint {
        match index {
            EntityRegistryIndex::Stable(i) => &mut self.state.stable_entities.points[i],
            EntityRegistryIndex::Unstable(i) => &mut self.state.unstable_entities.points[i],
        }
    }

    pub fn get_bone(&self, index: EntityRegistryIndex) -> &EntityBone {
        match index {
            EntityRegistryIndex::Stable(i) => &self.state.stable_entities.bones[i],
            EntityRegistryIndex::Unstable(i) => &self.state.unstable_entities.bones[i],
        }
    }

    pub fn get_joint(&self, index: EntityRegistryIndex) -> &EntityJoint {
        match index {
            EntityRegistryIndex::Stable(i) => &self.state.stable_entities.joints[i],
            EntityRegistryIndex::Unstable(i) => &self.state.unstable_entities.joints[i],
        }
    }
